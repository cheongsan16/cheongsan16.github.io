...
python bs4를 이용한 HTML 웹크롤링. 시작일과 종료일, 뉴스의 분야를 입력받아 해당 날짜에 올라온 해당 분야의 모든 뉴스기사를 제목과 함께 출력하는 코드를 제작하였다.
전에 시작일과 종료일을 입력받아 그 사이의 모든 날짜를 출력하는 코드를 짜두었기 때문에 그 코드를 활용하였다. 해당 코드의 링크는 [이곳](https://cheongsan16.github.io/docs/Python/date/).
처음으로 100줄이 넘어가는 코드를 짜보는 것이라 어렵기도 하였다. (물론 그 중 50줄은 시작일 종료일 코드가 차지하긴 했다.) 이번 프로젝트로 def에 대해 좀 더 이해하게 되었다. python에서 함수와 클래스, 딕셔너리는 아직은 많이 사용해보지 않아 낯설지만 (체감상) 어느 정도 규모가 있는 프로젝트를 진행할 때마다 찾아가며 코드를 짜는 재미가 있는 것 같다.
